...
```go
// Go：过滤 / 选择器 / 记忆，全部函数直调
import picktui "github.com/havoc-rao/picktui/go"
hits := picktui.FilterStructuredCandidates(cands, "ele dev", picktui.FilterOptions{})
```

```ts
// TS：同一引擎的效果，薄绑定经 JSON 协议调用
import { filter, pick, histGet } from '@havocrao/picktui';
...
│   └── integration/       接入文档：protocol.md（协议 v1 唯一契约）、ts.md（TS/JS 接入指南）
...
- 各包**独立管理版本、独立发布**：Go 模块按 git tag（`go/v0.1.0`）发布，npm 包按
  package.json 版本发布（`cd ts && npm publish`）；绑定可通过 `picktui version`
  校验引擎最低版本。
...
```console
$ cd go && go build ./cmd/picktui && go test ./...
```
...
// 只要过滤/高亮
hits := picktui.FilterStructuredCandidates(cands, "ele dev",
    picktui.FilterOptions{Mode: picktui.MatchSubstring})
ranges := picktui.HighlightRanges("electron:dev", "ele dev", opts)
v, ok := picktui.AutoResolve(cands, "re") // --auto 解析

// 只要交互选择器（调用方负责打开 /dev/tty）
res, err := picktui.Run(picktui.Options{Title: "pick", Cands: cands,
    FilterOpts: opts, StatusHint: "type to filter · enter select · esc cancel"}, tty)

// 只要选择记忆 / 首次确认
picktui.SavePick("npm run", "release")
picktui.LastPick(picktui.LoadPickHistory(), "npm run")
picktui.Confirm("npm run", "release")
picktui.IsConfirmed("npm run", "release")

// 完整 CLI 命令嵌入（宿主对齐：shr 即以此接入）
picktui.SetName("shr")               // 消息前缀/TUI 标题
picktui.SetOffEnv("SHR_PICK")        // 非交互开关环境变量
picktui.SetDataDir(core.ConfigDir()) // 记忆/确认文件目录
picktui.SetMapResolver(ResolvePickerMapArg) // --map 裸名解析钩子
os.Exit(picktui.Pick(args, os.Stdout, os.Stderr))
os.Exit(picktui.Menu(args, os.Stdout, os.Stderr))
```
...
```ts
import { filter, resolve, pick, histGet, histSet, confirmCheck, confirmAdd } from '@havocrao/picktui';
// 按模块 import（tree-shakable）：
// import { filter } from '@havocrao/picktui/filter'
// import { pick, menu } from '@havocrao/picktui/tui'
...
```console
$ go build -C go -o dist/picktui ./cmd/picktui

$ git branch | dist/picktui pick                              # 交互过滤选择
$ PICKTUI_PICK=off dist/picktui pick -q feat -1 <(git branch)  # 非交互确定性
$ dist/picktui _menu db mysql postgres                        # 菜单
$ printf 'dev\tDev\nbuild\tBuild\n' | dist/picktui filter --json --query de
$ printf 'build\nrelease\nserve\n' | dist/picktui resolve --json --query re
$ dist/picktui hist get "git p"; dist/picktui hist set "git p" pull
$ dist/picktui confirm check "npm run" release; dist/picktui confirm add "npm run" release
```

退出码：0 成功 / 1 运行错误 / 2 用法错误 / 130 取消。数据走 stdout、消息走
stderr。完整协议见 [docs/integration/protocol.md](docs/integration/protocol.md)。
...
| 包 | 位置 | 版本管理 |
|---|---|---|
| Go 引擎 | `go/`（github.com/havoc-rao/picktui/go） | git tag `go/v0.1.0`（多模块仓库前缀 tag，根 tag `v0.1.0` 亦可） |
| npm 包 | `ts/`（@havocrao/picktui） | package.json `version`（同号对齐，如 0.1.0） |

**本地开发（未发布）**：

```go
// Go 宿主（shr 等）：go.mod 加
replace github.com/havoc-rao/picktui/go => ../picktui/go
```

```jsonc
// TS/JS 宿主：package.json 加
{ "dependencies": { "@havocrao/picktui": "file:../picktui/ts" } }
// 任意宿主：把 go/cmd/picktui 构建的二进制放入 PATH，或设 PICKTUI_BIN 指过去
```
