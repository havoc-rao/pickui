# picktui 抽取计划（docs/plan）— 多语言可复用的 TUI 候选选择器

> 目标：把 shr 的「TUI 辅助选择执行 CLI」整理为独立项目 **picktui**——
...
候选来源 → 过滤 → TUI 选择 → 输出选中值。本次把它抽为独立项目 **picktui** 并发布。

与纯 Go 库方案的关键差异：TUI 是终端渲染问题，无法用「三个语言各写一套」保证
同样效果。因此 picktui 采用**引擎 + 协议 + 绑定**三层架构：

```
┌─ 宿主项目 ────────────────────────────────────────────┐
│  Go 项目：import github.com/havoc-rao/picktui/go      │
│    ├─ filter（纯函数）   tui（选择器）  history/confirm  │
│    └─ Pick/Menu（完整 CLI 命令嵌入）                    │
│  TS/JS 项目：npm 包 @havocrao/picktui                  │
│    ├─ filter()           pick()          history()     │
...
└──────────────┬──────────────────────────┬──────────────┘
               ...
               ▼                          ▼
   ┌─────────────────────────────────────────────────────┐
   │ picktui 引擎（Go 单二进制，无运行时依赖，权威实现）       │
...
> `go/`（Go 引擎模块 `github.com/havoc-rao/picktui/go`，含引擎库与 `cmd/picktui`
> 二进制）、`ts/`（npm 包 `@havocrao/picktui`，esm+cjs+d.ts 双格式同时服务
...
### 2.1 移入 picktui（从 shr 复制并泛化）

| shr 源文件 | picktui 落点（均在 `go/` 子目录） | 内容 |
|---|---|---|
...
- 协议细节（字段、示例、错误约定）单独维护 `docs/integration/protocol.md`。

## 4. 语言绑定设计（按需食用）

### 4.1 Go 绑定 = 引擎本体

`github.com/havoc-rao/picktui/go` 包内按需取用，无进程开销：
...
npm 包 `@havocrao/picktui`（esm + cjs 双输出，模块按需 import）：

| 模块 | 能力 | 实现 |
|---|---|---|
| `filter` | 过滤 + 高亮区间 + AutoResolve | 调引擎 `filter/resolve --json`；可选内嵌同构 JS 实现（`PICKTUI_NO_BIN=1` 时启用，纯数据无 TUI） |
...
- 引擎发现顺序：`$PICKTUI_BIN` → 平台 optionalDependencies 二进制包 → `PATH`。
...
| 需求 | Go | TS/JS |
|---|---|---|
| 只要过滤/高亮 | `FilterStructuredCandidates` | `import { filter } from '@havocrao/picktui/filter'` |
...
## 5. 泛化点（shr 硬编码 → picktui 可配置）

| shr 硬编码 | picktui 泛化 |
|---|---|
| 命令前缀 `shr `/`shr:` | `SetName()`，默认 `picktui` |
| 环境变量 `SHR_PICK=off` | `SetOffEnv()`，默认 `PICKTUI_PICK` |
| 数据目录 `~/.config/shr` | `SetDataDir()`，默认 `$XDG_CONFIG_HOME/picktui` |
...
4. `cli/setup.go`、`cli/pickers.go` 改 import：`core.Candidate → picktui.Candidate`、
   `core.FilterOptions → picktui.FilterOptions`、`tui.Run → picktui.Run`。
5. `tests/pickers_test.go` 依赖的 `shrConfigDir` 随迁走 → shr 侧新增
   `tests/helper_test.go` 保留。
6. `go.mod`（shr 侧）：`require github.com/havoc-rao/picktui/go v0.1.0` +
   `replace => ../picktui/go`（发布后移除 replace）。
7. 验收：`go build ./... && go test ./... -count=1` 全绿；冒烟 `shr pick`（交互+非交互）
   与 `shr _menu`。

## 7. 测试策略

- picktui 独立测试：迁移 + 适配现有全部用例（filter 匹配矩阵、TUI 按键行为、
...
1. 用户创建 GitHub 空仓库 `havoc-rao/picktui`（不要 README/license）。
2. 提交（含 docs/dev/plan.md、docs/integration/protocol.md）→ push → 打 `v0.1.0` tag。
3. shr 移除 replace，依赖 `github.com/havoc-rao/picktui/go v0.1.0`，全量测试后单独 commit。
4. M2：TS/JS 绑定入库（`ts/`），`npm publish`（需 npm 账号/token，
   用户执行或提供；包名 `@havocrao/picktui`）。
...
| TS 绑定与引擎版本错配 | 协议文档化 + 绑定声明最低引擎版本，`picktui --version` 校验 |
| TUI 效果跨语言不一致 | 渲染/按键/过滤唯一实现在引擎；绑定层禁止自绘或重实现匹配 |
| 双仓库/多包后续开发不同步 | 引擎改动统一进 picktui；shr 只保留薄包装 |