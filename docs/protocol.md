...
1. `$PICKUI_CONFIG_DIR`（显式覆盖）
2. `$XDG_CONFIG_HOME/pickui`
3. `~/.config/pickui`

状态文件：`history.toml`（选择记忆，含旧 `picks.toml` 自动迁移）、`confirm.toml`
（首次确认记录）。非交互开关：`$PICKUI_PICK=off` 时 `pick` 跳过 TUI
（确定性的脚本语义），`--auto` 同时跳过首次确认询问。

## pick

```
pickui pick [flags] [cand...]
git branch | pickui pick
pickui pick --from "git branch"
pickui pick --from 'npm run' --map 'node .../npm-run.mjs' --label 'npm run' --auto -q re
```
...
| `--auto` | 配合 `-q`：精确或唯一前缀匹配直接输出；首次匹配该 (label, value) 需经 `/dev/tty` 确认（记录 confirm.toml）；`$PICKUI_PICK=off`/无 tty/无 label 跳过确认；无法唯一确定回退 TUI |

无 TTY 退化：无 query 取首个候选；有 query 过滤后取首个匹配，
**无匹配报错退出 1**（绝不静默回退首个，避免脚本拿到毫不相干的选中值）。
`--auto` 无唯一解时报错退出 1。候选为空报 `pickui pick: no candidates` 退出 1。

## menu

```
pickui menu <label> <cand...>
```
...
```
echo -e "build\tCompile\ndev\tServer" | pickui filter --json --query "de" [--mode substring|token|fuzzy] [--sep <chars>]
```
...
```console
$ printf 'electron:dev\nelectron:build\nmydev\n' | pickui filter --json --query 'ele dev'
[{"value":"electron:dev","desc":"","ranges":[[0,3],[9,12]]}]

$ printf 'electron:dev\nelectron:build\n' | pickui filter --json --query 'ele dev' --mode token --sep ':_' | jq -c '.[].value'
"electron:dev"
```
...
```
pickui hist set "git p" pull     # 成功：stdout 空，退出 0；错误：stderr + 退出 1
pickui hist get "git p"          # {"label":"git p","last":"pull"}，退出 0
pickui hist get "never set"      # {"label":"never set","last":""}，退出 0
```
...
```
pickui confirm check "npm run" release   # {"confirmed":false} | {"confirmed":true}
pickui confirm add  "npm run" release    # 幂等；输出确认后的实际状态 {"confirmed":true}
pickui confirm add  "" x                 # label 为空不落盘 → {"confirmed":false}
```
...
`pickui version`（或 `-v/--version`）输出 `pickui <semver>`（如 `pickui 0.1.0`）。
...
- 字段名/形状/退出码为协议 v1 的一部分，仅在 v2（大版本）破坏性变更；
  新增字段只增不删，旧绑定忽略新字段。
- 引擎改动统一进 pickui 仓库；宿主（shr 等）只保留薄包装与品牌对齐
