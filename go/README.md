...
模块：`github.com/havoc-rao/pickui/go`，包名 `pickui`。引擎本体，三种消费方式：

1. **嵌入**：`import pickui "github.com/havoc-rao/pickui/go"`，函数直调（同进程，无进程开销）
2. **二进制**：`go build ./cmd/pickui` → 协议子命令（见 ../docs/protocol.md）
...
非交互（`$PICKUI_PICK=off` 或无 /dev/tty）：无 query 取首个，有 query 过滤取首。
...
DataDir 解析、Pick/Menu 非交互路径、协议子命令 JSON 往返（cmd/pickui）。