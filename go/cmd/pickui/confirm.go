...
//
//	pickui confirm check <label> <value>  → {"confirmed":true|false}
//	pickui confirm add <label> <value>    → 记录后输出 {"confirmed":<实际状态>}
...
func cmdConfirm(stdout, stderr io.Writer, args []string) int {
	if len(args) < 3 {
		fmt.Fprintf(stderr, "usage: pickui confirm %s\n", confirmUsage(args))
		return 2
	}
	sub, label, value := args[0], args[1], args[2]
	switch sub {
	case "check":
		if err := encodeJSON(stdout, map[string]bool{"confirmed": pickui.IsConfirmed(label, value)}); err != nil {
			fmt.Fprintf(stderr, "pickui confirm: 编码 JSON 失败: %v\n", err)
			return 1
		}
		return 0
	case "add":
		if err := pickui.Confirm(label, value); err != nil {
			fmt.Fprintf(stderr, "pickui confirm: %v\n", err)
			return 1
		}
		if err := encodeJSON(stdout, map[string]bool{"confirmed": pickui.IsConfirmed(label, value)}); err != nil {
			fmt.Fprintf(stderr, "pickui confirm: 编码 JSON 失败: %v\n", err)
