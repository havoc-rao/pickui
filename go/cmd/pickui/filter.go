// filter — 协议子命令：纯函数数据面。
//
//	pickui filter --json [--query <q>] [--mode substring|token|fuzzy] [--sep <chars>]
//
// stdin 按结构化候选读取（key<TAB>description，规则见 pickui.Candidate），
// stdout 输出 JSON 数组：
//
...
func cmdFilter(stdin io.Reader, stdout, stderr io.Writer, args []string) int {
	...
	for i := 0; i < len(args); i++ {
		...
		case len(a) > len("--sep=") && a[:len("--sep=")] == "--sep=":
			sep = a[len("--sep="):]
		default:
			fmt.Fprintf(stderr, "pickui filter: unknown flag %q\n", a)
			return 2
		}
	}
	if !seenJSON {
		fmt.Fprintln(stderr, "pickui filter: missing --json (protocol requires explicit --json)")
		return 2
	}
	opts, err := parseFilterOptions(mode, sep)
	if err != nil {
		fmt.Fprintf(stderr, "pickui filter: %v\n", err)
		return 2
	}

	cands := pickui.StructuredFromReader(stdin)
	hits := pickui.FilterStructuredCandidates(cands, query, opts)
	out := make([]filterResult, 0, len(hits))
	for _, c := range hits {
		ranges := pickui.HighlightRanges(c.Value, query, opts)
		if ranges == nil {
			ranges = [][2]int{}
		}
	...
	if err := encodeJSON(stdout, out); err != nil {
		fmt.Fprintf(stderr, "pickui filter: 编码 JSON 失败: %v\n", err)
		return 1
	}
	return 0
}
...
func parseFilterOptions(mode, sep string) (pickui.FilterOptions, error) {
	opts := pickui.FilterOptions{Mode: pickui.MatchSubstring}
	switch mode {
	case "", "substring":
		// 默认
	case "token":
		opts.Mode = pickui.MatchTokenPrefix
		opts.Sep = sep
	case "fuzzy":
		opts.Mode = pickui.MatchFuzzy
	default:
