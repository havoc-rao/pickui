...
//
//	pickui resolve --json [--query <q>] [--mode substring|token|fuzzy] [--sep <chars>]
//
...
func cmdResolve(stdin io.Reader, stdout, stderr io.Writer, args []string) int {
	...
	for i := 0; i < len(args); i++ {
		...
		case len(a) > len("--sep=") && a[:len("--sep=")] == "--sep=":
			sep = a[len("--sep="):]
		default:
			fmt.Fprintf(stderr, "pickui resolve: unknown flag %q\n", a)
			return 2
		}
	}
	if !seenJSON {
		fmt.Fprintln(stderr, "pickui resolve: missing --json (protocol requires explicit --json)")
		return 2
	}
	if _, err := parseFilterOptions(mode, sep); err != nil {
		fmt.Fprintf(stderr, "pickui resolve: %v\n", err)
		return 2
	}

	cands := pickui.StructuredFromReader(stdin)
	v, ok := pickui.AutoResolve(cands, query)
	if !ok {
		return 1 // stdout 保持为空（"空 + 退出码 1" 协议）
	}
	if err := encodeJSON(stdout, map[string]string{"value": v}); err != nil {
		fmt.Fprintf(stderr, "pickui resolve: 编码 JSON 失败: %v\n", err)
		return 1
	}
	return 0
}
