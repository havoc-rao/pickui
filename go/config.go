// Package picktui：可供复用的 TUI 辅助候选选择器（fzf 风格）。
//
// 核心能力：
...
var (
	cmdName     = "picktui"          // 提示/错误前缀中的命令名
	offEnv      = "PICKTUI_PICK"     // 非交互开关环境变量（值 "off" 时跳过 TUI）
	dataDir     = ""                // 显式数据目录（空 = 按默认规则解析）
	mapResolver func(string) string // --map 值解析钩子（默认原样返回）
)

// SetName 设置提示/错误前缀中的命令名（默认 "picktui"）。shr 嵌入时设为 "shr"，
...
// （确定性脚本语义）。默认 "PICKTUI_PICK"；shr 嵌入时设为 "SHR_PICK"。
...
// 默认解析优先级：$PICKTUI_CONFIG_DIR → $XDG_CONFIG_HOME/picktui → ~/.config/picktui。
...
func DataDir() string {
	...
	if d := os.Getenv("PICKTUI_CONFIG_DIR"); d != "" {
		return d
	}
	...
	return filepath.Join(base, "picktui")
}
