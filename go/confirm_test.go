// Package picktui_test: 自动匹配首次确认记录（confirm.toml）测试。
...
func TestConfirmStateRoundTrip(t *testing.T) {
	...
	if picktui.IsConfirmed("npm run", "release") {
		t.Fatal("fresh state should not be confirmed")
	}
	if err := picktui.Confirm("npm run", "release"); err != nil {
		t.Fatal(err)
	}
	if !picktui.IsConfirmed("npm run", "release") {
	...
	// 其它 label 或值不受影响
	if picktui.IsConfirmed("npm run", "dev") {
		t.Fatal("unconfirmed value leaked")
	}
	if picktui.IsConfirmed("docker exec", "release") {
...
func TestConfirmIdempotent(t *testing.T) {
	picktuiConfigDir(t)
	for i := 0; i < 3; i++ {
		if err := picktui.Confirm("git co", "main"); err != nil {
			t.Fatal(err)
		}
	}
	// 同一 (label, value) 只记录一次
	if got := len(picktui.LoadConfirmed()["git co"]); got != 1 {
...
func TestConfirmMultipleValuesPerLabel(t *testing.T) {
	picktuiConfigDir(t)
	if err := picktui.Confirm("npm run", "dev"); err != nil {
		t.Fatal(err)
	}
	if err := picktui.Confirm("npm run", "build"); err != nil {
		t.Fatal(err)
	}
	if err := picktui.Confirm("docker exec", "web"); err != nil {
		t.Fatal(err)
	}
...
func TestConfirmEmptyLabelNoop(t *testing.T) {
	dir := picktuiConfigDir(t)
	if err := picktui.Confirm("", "x"); err != nil {
		t.Fatal(err)
	}
	// 无 label 不落盘（无状态可记）
...
func TestConfirmCorruptedFileTolerated(t *testing.T) {
	...
	if got := len(picktui.LoadConfirmed()); got != 0 {
