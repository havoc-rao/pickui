...
const { histGet, histSet, confirmCheck, confirmAdd } = pickui;
...
test('hist set 失败（数据目录被文件占用）→ PickuiError', async () => {
  const { mkdtempSync, writeFileSync } = await import('node:fs');
  const { tmpdir } = await import('node:os');
  const dir = mkdtempSync(join(tmpdir(), 'pickui-blocked-'));
  const blocked = join(dir, 'not-a-dir');
  writeFileSync(blocked, 'x');
  const prev = process.env.PICKUI_CONFIG_DIR;
  process.env.PICKUI_CONFIG_DIR = blocked;
  try {
